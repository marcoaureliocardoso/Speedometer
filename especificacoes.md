...
* **Avisos relativos ao limite da via:** quando houver `roadSpeedLimit` confiável, disponível e apto a TTS (online ou offline fresco), o aplicativo deve anunciar `Velocidade abaixo da metade do limite da via.` quando a velocidade bruta entrar na faixa estritamente inferior a `roadSpeedLimit / 2`; deve anunciar `Atenção: acima do limite de velocidade.` quando a velocidade bruta entrar na faixa estritamente superior a `roadSpeedLimit`. Nos valores exatamente iguais a metade do limite ou ao limite legal, não há aviso. Esses alertas só são elegíveis nos modos `limites apenas` e `limites e faixas de 5 km/h`; no modo `silencioso`, nenhum deles pode ser emitido.
* **Confirmação e rearme dos avisos relativos:** a entrada em cada faixa deve ser confirmada por uma segunda leitura bruta válida, consecutiva, ainda na mesma faixa; os requisitos de `speedAccuracy`, ou sua regra alternativa, aplicam-se a essa confirmação. Após a fala de velocidade baixa, o aviso só pode ser rearmado depois de duas leituras válidas em velocidade maior ou igual a `roadSpeedLimit / 2 + 2 km/h`. Após a fala de excesso, só pode ser rearmado depois de duas leituras válidas em velocidade menor ou igual a `roadSpeedLimit - 2 km/h`. Quando o limite confiável mudar, os dois estados devem ser recalculados a partir das leituras seguintes; o alerta de mudança de limite tem precedência e suprime avisos relativos por 10 segundos.
* **Arbitragem de áudio:** O `AudioAnnouncementCoordinator` deve manter no máximo uma fala ativa e não deve manter fila de alertas. A prioridade é: (1) alerta de mudança de limite, (2) alerta de excesso, (3) alerta de velocidade abaixo da metade do limite e (4) alerta de faixa de 5 km/h. Um novo alerta de prioridade 1 ou 2 pode cancelar uma fala de prioridade menor, descarta alertas pendentes de prioridade menor e substitui alerta ativo da mesma prioridade. Alertas de prioridade 3 e 4 nunca interrompem fala ativa de prioridade superior ou igual.
...
* **Terminologia da escala:** neste requisito, `roadSpeedLimit` representa o limite legal confiável da via. Quando ele existir, a escala visual terá `gaugeMin = roadSpeedLimit / 2` e `gaugeMax = roadSpeedLimit`. Os nomes `gaugeMin` e `gaugeMax` devem ser usados na interface e no código para não confundir o teto do medidor com o limite legal.
* **Composição do painel em condução:** a tela deve priorizar, nesta ordem: (1) velocidade numérica, (2) estado/valor do limite, (3) ponteiro radial, (4) estado de rastreamento e degradações e (5) nome da via, origem e data. O valor do limite deve ser exibido como `Limite: X km/h` quando disponível; o rótulo isolado "Limite indisponível" deve ser usado nos demais casos. O nome da via é opcional e sua ausência não pode causar espaço vazio relevante ou alterar a posição dos elementos principais.
* **Faixa abaixo da escala:** quando a velocidade estiver abaixo de `gaugeMin`, o número principal deve continuar exibindo a velocidade real e o ponteiro deve ficar no início da escala. A interface deve indicar de forma textual que o ponteiro está no mínimo da escala; não deve sugerir que o veículo está parado.
* **Excesso de velocidade:** além do ponteiro vermelho pulsante, o painel deve mostrar o ícone de alerta e o texto `Acima do limite`. O pulso deve respeitar a preferência de redução de movimento do sistema: quando ela estiver ativa, deve usar destaque estático. Vermelho nunca pode ser o único sinal de excesso.
* **Origem e validade:** quando o limite estiver disponível, o painel deve exibir `OSM online` ou `Limite offline · DD/MM/AAAA`. Para dado offline desatualizado, deve acrescentar `Dados offline desatualizados` e manter o limite apenas como informação visual, sem sugerir que haverá alerta por voz.
* **Aviso legal:** o texto `A sinalização oficial da via prevalece.` deve permanecer acessível no painel de condução, como texto secundário fixo ou por um botão de informações claramente rotulado. Ele não pode ser ocultado exclusivamente em um menu de configurações.
* **Estados de carregamento e indisponibilidade:** enquanto a primeira posição válida é aguardada, o painel deve mostrar `Aguardando GPS` sem exibir um limite. Após três segundos sem posição válida, deve exibir `Localização desatualizada` junto ao último número conhecido; após dez segundos, deve trocar o limite para `Limite indisponível`. Fora do Brasil, perto da fronteira, ou com o dataset de fronteira inválido, deve exibir somente a velocidade e `Limite indisponível`.
...
### RF-008: Arquitetura de Interface e Estados de Apresentação

* **Telas mínimas:** a aplicação deve disponibilizar `Painel parado`, `Painel em rastreamento`, `Consentimento de dados`, `Permissão/localização`, `Configurações`, `Regiões offline`, `Detalhe de região`, `Nova região`, `Importar extrato`, `Diagnóstico` e `Sobre`. A navegação para configurações, regiões, diagnóstico e Sobre deve ficar disponível somente fora do rastreamento ativo.
* **Painel parado:** deve oferecer um controle principal grande, com rótulo `Iniciar rastreamento`. Antes da primeira ativação, a sequência deve apresentar o consentimento do modo de dados e, quando necessário, as permissões de localização. A opção `Cancelar` deve retornar ao painel parado sem iniciar serviço, GPS em segundo plano ou consulta de rede.
* **Painel em rastreamento:** deve exibir um único controle operacional primário, `Encerrar rastreamento`, com confirmação apenas quando a ação puder descartar uma construção ou operação pendente. Não deve expor controles de configuração detalhada, seleção de região, busca, abertura do OSM ou importação. Indicadores e o botão de informações podem ser consultados sem interromper o rastreamento.
* **Permissões e serviço de localização:** para permissão temporariamente negada, a tela deve explicar por que a localização precisa é necessária e oferecer `Tentar novamente` e `Continuar sem rastrear`. Para negativa permanente, deve oferecer `Abrir configurações do Android` e `Continuar sem rastrear`. Para serviço de localização desativado, deve oferecer `Abrir configurações de localização` e `Continuar sem rastrear`. Nenhum desses estados pode apresentar uma ação que simule rastreamento ativo.
* **Consentimentos de rede:** o consentimento para rastreamento online, geocodificação, seleção de mapa externa e construção regional deve identificar separadamente o destino, a finalidade e quais dados são enviados. Cada diálogo deve ter aceitação explícita, recusa e acesso à política de privacidade local; fechar o diálogo equivale a recusar. O consentimento de construção não pode ser usado para habilitar rastreamento online.
* **Regiões offline:** a lista deve exibir, por região, nome/origem, área, espaço ocupado, cobertura concluída, data da última coleta, células pendentes/com falha e estado (`ativa`, `pausada`, `pausada por quota`, `aguardando Wi-Fi`, `falhou` ou `desatualizada`). O detalhe deve oferecer ações contextualizadas de pausar, retomar, cancelar, atualizar e remover. Remoção e limpeza total devem exigir confirmação e informar o dado local que será apagado.
* **Nova região e importação:** o fluxo de nova região deve usar passos explícitos: selecionar o ponto, definir raio/área, revisar prévia, obter consentimento de construção e confirmar. O passo de prévia deve mostrar área, células, espaço e tempo estimados, além de restrições de Wi-Fi, quota e sobreposição. O fluxo de importação deve mostrar validação, progresso determinável quando possível, possibilidade de cancelar e resultado; em sobreposição deve apresentar `Substituir` e `Manter separado`, com a consequência de cada escolha e `Substituir` selecionado por padrão.
* **Estados de degradação:** cada razão deve possuir texto, ícone, severidade e, quando aplicável, ação. A ordem de prioridade visual é: `offlineDataResetRequired`, `countryBoundaryUncertain` e `locationStale`; depois `ttsUnavailable` e `audioUnavailable`; depois `gpsWeak`, `roadMatchLowConfidence`, `overpassUnavailable`, `offlineCoverageMissing`, `offlineDataStale` e `onlineDataDisabled`. A UI pode agrupar razões de baixa severidade em um resumo acessível, mas não pode ocultar uma razão de prioridade alta.
* **Mapa de razões para apresentação:** `gpsWeak` = `GPS com baixa precisão`; `locationStale` = `Localização desatualizada`; `offlineDataStale` = `Dados offline desatualizados`; `offlineCoverageMissing` = `Sem cobertura offline`; `roadMatchLowConfidence` = `Via não confirmada`; `overpassUnavailable` = `Consulta online indisponível`; `onlineDataDisabled` = `Modo somente offline`; `audioUnavailable` = `Áudio indisponível`; `ttsUnavailable` = `Voz pt-BR indisponível`; `offlineDataResetRequired` = `Dados offline precisam ser reconstruídos`; `countryBoundaryUncertain` = `Limite indisponível perto da fronteira`. Apenas razões com ação devem exibir botão associado.
* **Diagnóstico e Sobre:** diagnóstico deve apresentar somente os contadores agregados permitidos por RNF-010, uma prévia antes de compartilhar e ações de voltar/cancelar. A tela Sobre deve expor versão do app, escopo Brasil/pt-BR, o aviso de sinalização oficial e a atribuição/link do OpenStreetMap.
* **Retorno e persistência visual:** ao voltar de configurações ou regiões, a interface deve preservar o painel parado e a seleção já confirmada. Se o usuário retornar durante uma operação de construção, o estado e o progresso devem continuar visíveis na lista e na notificação, sem depender de a tela permanecer aberta.

### RF-009: Sistema Visual, Interação e Recuperação de Erros

* **Tema e tokens:** o painel de condução deve iniciar em tema escuro para reduzir ofuscamento. A implementação deve usar tokens semânticos (`background`, `surface`, `onSurface`, `primary`, `warning`, `critical` e `success`), e não cores fixas por componente. O tema escuro usa `background #0B1015`, `surface #17212B`, `onSurface #F1F5F9` e `primary #75D5F7`; o claro usa `background #F7F9FC`, `surface #FFFFFF`, `onSurface #18212B` e `primary #006782`. Todo par texto/fundo e ícone essencial/fundo deve atender ao contraste mínimo de RNF-007 nos dois temas. Estados devem combinar cor, ícone e texto; cores não têm significado isolado.
* **Largura e adaptação:** a UI deve suportar largura útil a partir de 320 dp. Em até 599 dp, deve usar a composição compacta de uma coluna; de 600 a 839 dp, pode ampliar os espaçamentos mantendo uma coluna; em 840 dp ou mais, telas fora de condução podem usar duas colunas. Em paisagem durante rastreamento, deve usar a composição do medidor à esquerda e informações à direita, sem depender de largura mínima maior que 568 dp.
* **Ritmo e alvos de toque:** a grade base é de 8 dp. Controles acionáveis devem ter área de toque mínima de 48 × 48 dp, separação mínima de 8 dp e foco visível. O botão de iniciar/encerrar rastreamento deve ocupar toda a largura útil, ter altura mínima de 56 dp e permanecer afastado de gestos de borda e da barra de navegação.
* **Tipografia e escala:** além do valor de velocidade com mínimo de 96 sp, devem ser usados estilos semânticos para limite (32 sp), estado/ação principal (20 sp), título de tela (24 sp), corpo (16 sp) e apoio (14 sp). Esses valores são bases antes da escala do sistema; nenhum texto essencial pode ser truncado em vez de quebrar linha ou reorganizar a composição.
* **Referência de composição do painel:** em retrato, a ordem vertical obrigatória é: barra compacta de estado/ajuda; velocidade e unidade; medidor radial; limite e aviso de excesso ou indisponibilidade; origem/validade e aviso legal; controle de rastreamento. A referência é:

```text
[ Estado de rastreamento                 Informações ]
                         72
                        km/h
                    ( medidor radial )
                   Limite: 80 km/h
...
* **Escala radial:** o arco deve ocupar 270 graus, com início no canto inferior esquerdo e fim no canto inferior direito. Deve exibir marcas maiores em incrementos de 5 km/h quando `gaugeMax - gaugeMin` for até 40 km/h, de 10 km/h quando for até 80 km/h e de 20 km/h nos demais casos; os rótulos numéricos obrigatórios são `gaugeMin` e `gaugeMax`. A escala nunca deve ocultar o valor numérico real. Quando não houver limite confiável, o arco, marcas e ponteiro devem ser substituídos por espaço reservado neutro com o texto `Limite indisponível`; não pode parecer um medidor com valor zero.
* **Variações do painel:** em `abaixo da metade`, o ponteiro fica no início da escala, há rótulo `Abaixo da faixa do medidor` e não há uso de cor de alerta. Em `acima do limite`, o ponteiro fica no fim, com rótulo e ícone `Acima do limite`. Em `aguardando GPS`, o painel mostra `Aguardando GPS` e esqueletos somente para limite/origem. Em `localização desatualizada`, mantém a última velocidade marcada como `última leitura`; em `limite indisponível`, preserva a velocidade e remove toda indicação de limite legal. Essas variações não podem reposicionar o botão de encerrar rastreamento.
* **Ações por degradação:** durante rastreamento, os indicadores somente explicam o estado e não iniciam fluxos de configuração. Fora dele: `ttsUnavailable` oferece `Abrir configurações de voz`; `offlineDataResetRequired` oferece `Reconstruir dados offline` e `Limpar dados offline`; `offlineCoverageMissing` e `offlineDataStale` oferecem `Gerenciar regiões`; `onlineDataDisabled` oferece `Alterar modo de dados`. `gpsWeak`, `locationStale`, `roadMatchLowConfidence`, `overpassUnavailable`, `audioUnavailable` e `countryBoundaryUncertain` são informativos e não devem apresentar ação enganosa.
* **Configurações de voz:** o modo padrão é `limites apenas`, o volume relativo padrão é 100% e a velocidade de fala padrão é 1,0×. Volume deve variar de 0% a 100% em incrementos de 5%; velocidade de 0,5× a 1,5× em incrementos de 0,1×. Cada seletor deve apresentar o valor textual atual e a tela deve oferecer `Ouvir exemplo` somente fora do rastreamento.
* **Notificações de construção:** a notificação persistente de construção deve exibir `Atualizando limites offline`, células concluídas/total, percentual e estimativa de conclusão quando calculável. Nos estados pausados, deve substituir a estimativa pelo motivo (`Aguardando Wi-Fi`, `Quota insuficiente`, `Pausada pelo rastreamento` ou `Aguardando desbloqueio`). As únicas ações permanecem `Pausar` e `Cancelar`; erro e conclusão devem abrir o detalhe da região, sem ação automática.
* **Estados vazios e de falha das regiões:** sem região cadastrada, a tela deve mostrar `Nenhuma região offline`, explicar que limites locais podem ser preparados para uso sem conexão e oferecer `Criar região`. Com cobertura incompleta, deve mostrar `Cobertura em andamento` e a ação `Retomar` quando aplicável. Após falha definitiva, deve mostrar `Algumas células precisam de atenção`, quantidade afetada e `Tentar novamente`. Com quota esgotada, deve mostrar `Espaço reservado insuficiente`, a quota e o espaço ocupado, além de `Revisar armazenamento`; não deve sugerir remoção automática de dados.
* **Navegação e retornos do sistema:** o botão Voltar nas telas de configuração, regiões, diagnóstico e Sobre retorna ao painel parado e preserva alterações já confirmadas. Em formulários de nova região ou importação com mudanças não confirmadas, deve solicitar descarte. O botão Voltar enquanto há rastreamento ativo envia o aplicativo ao segundo plano e não encerra o serviço. Após retorno de configurações Android, seletor de arquivos ou mapa externo, a tela de origem deve revalidar permissões e dados, mostrar o resultado e manter a entrada manual como alternativa em falha ou cancelamento.
* **Foco e leitura de tela:** a ordem de foco do painel é estado de rastreamento, velocidade, limite, origem/validade, degradações, informações e controle principal. O medidor deve expor uma única descrição semântica, incluindo velocidade, limite quando disponível e estado de excesso/indisponibilidade; marcas individuais não devem criar uma sequência longa no TalkBack.
...
* **RNF-007 (Acessibilidade visual):** A velocidade numérica deve ter tamanho mínimo de 96 sp em orientação retrato. Textos e indicadores essenciais devem possuir contraste mínimo de 4,5:1 contra o fundo; a cor vermelha de excesso nunca pode ser o único indicador do estado. Todos os controles, indicadores de degradação, valor do limite e estado do rastreamento devem possuir rótulos semânticos em pt-BR para TalkBack. A tela deve permanecer utilizável com escala de fonte do sistema de 200%, sem sobreposição, corte de texto essencial ou perda do controle de encerrar rastreamento. Em orientação paisagem, o valor de velocidade, o estado do limite e o controle de encerrar rastreamento devem continuar simultaneamente visíveis; informações secundárias podem ser recolhidas em painel acessível. Animações não podem depender exclusivamente de movimento para comunicar estado e devem respeitar a preferência do sistema para redução de movimento.
...
18. **Jornadas e acessibilidade de interface:** Testes de widget e integração devem validar as jornadas de painel parado, consentimento recusado/aceito, permissões temporária e permanentemente negadas, localização desativada, início e encerramento de rastreamento, regiões offline, prévia de nova região, importação cancelada e conflito de sobreposição. Durante rastreamento, não podem estar disponíveis controles de configuração, seleção de região, busca, abertura de OSM ou importação. Devem validar também o mapa textual dos motivos de degradação, o aviso de excesso não dependente apenas da cor, preferência de redução de movimento, rótulos TalkBack em pt-BR, fonte do sistema em 200% e orientação paisagem sem ocultar velocidade, limite ou o controle de encerrar rastreamento.
19. **Avisos relativos ao limite:** Testes unitários devem validar, com `roadSpeedLimit = 40 km/h`, que duas leituras válidas consecutivas abaixo de 20 km/h emitem uma única fala de velocidade abaixo da metade; que 20 km/h não emite essa fala; e que o aviso só rearma após duas leituras de ao menos 22 km/h. Devem validar também que duas leituras válidas consecutivas acima de 40 km/h emitem uma única fala de excesso; que 40 km/h não emite essa fala; e que o aviso só rearma após duas leituras de no máximo 38 km/h. Testes de integração devem assegurar ausência de fala sem limite confiável, com dado offline desatualizado, em modo silencioso ou com localização desatualizada, supressão de 10 segundos após mudança de limite e a prioridade de mudança de limite, excesso, velocidade baixa e faixa de 5 km/h.
20. **Sistema visual e recuperação:** Testes de widget devem validar os dois temas, contraste de textos e ícones essenciais, alvos de toque de no mínimo 48 dp, composição estável em todos os cinco estados do painel, escala radial com seus rótulos e incrementos, valores abaixo/acima dos limites visuais e ausência de falsa escala quando o limite for indisponível. Devem validar os valores padrão, limites e incrementos de voz, conteúdo de notificação de construção e cada ação de degradação permitida fora do rastreamento. Testes de integração devem validar botão Voltar, descarte de formulário não confirmado, manutenção do rastreamento ao enviar o app ao segundo plano e o retorno de configurações Android, seletor de arquivo e mapa externo. Testes de acessibilidade devem verificar ordem de foco, descrição única do medidor e ausência de marcas individuais da escala na navegação TalkBack.
